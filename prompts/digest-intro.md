...
3. BILIBILI section — list each Bilibili UP主 with new videos (Chinese content)
...
### Bilibili video formatting
- Use the UP主's name as a section header (e.g. "稚晖君", "3Blue1Brown中文")
- Under each UP主, list each new video with its title and summary
- Include the video title in Chinese (original language)
- Include the direct link to the video from the JSON `url` field
- If the video has a transcript, provide a more detailed summary
- If the video has no transcript, provide a brief overview based on title and description
